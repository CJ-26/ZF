...
export function renderMixin(Vue){
  ...
  Vue.prototype._s= function(val){ //解析{{}}表达式，传化为字符串
    ...
    return val==null?"":(typeof val =="object")?JSON.stringify(val):val; //这里的val的值必须要JSON.stringify()。否则val为Object类型在页面上显示为:[object Object]
  
  }
  Vue.prototype._render = function(){  //调用render的自定义方法，生成虚拟节点
     const vm = this;
     let render = vm.$options.render;  //获取编译后render方法； // rendre就是compileToFunction（）返回的fn
